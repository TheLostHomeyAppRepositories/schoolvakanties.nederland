...
class MyApp extends Homey.App {
  ...
  async onInit() {
    ...
    isHolidayCondition.registerRunListener(async (args, state) => {
      let holidayDates;
      const regions = await this.filterRegion();
      holidayDates = this.processData(regions, args.region)
      return this.isHoliday(holidayDates);
    });
  }
  ...
  processData(regions, regionToFilter) {
    ...
    regions.map((vacation) => {
      const regionObj = vacation.regions.filter(region =>
  ...
  async filterRegion() {
    const data = await this.getHolidays()
    return data;
  }
