...
class MyApp extends Homey.App {
  ...
  createRangeDates(startDate, endDate) {
    ...
    array.map(m => {
      this.holidayDates.push(m.format("YYYY-MM-DD")); /*or any format you like*/
    });
  }
  ...
  filterRegion(regionToFilter) {
    if (!this.holidayData) {
      this.getHolidays().then(regions => {
        this.holidayData = regions;
        regions.map(vacation => {
          const region = vacation.regions.filter(region => region.region === regionToFilter)[0];
          this.holidayRegionDates.push(region);
        });
        this.holidayRegionDates.map(obj => this.createRangeDates(obj.startdate, obj.enddate))
      })
    }
  }
