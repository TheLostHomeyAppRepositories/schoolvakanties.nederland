...
class MyApp extends Homey.App {
  ...
  async onInit() {
    ...
    isHolidayCondition.registerRunListener(async (args, state) => {
      console.log('isHolidayCondition', args, state)
      let isHoliday;
      this.filterRegion(args.name).then((dates) => {
        console.log('dates', dates);
        isHoliday = this.isHoliday(dates);
        console.log('isHoliday', isHoliday);
      });
      console.log('return', isHoliday)
      return isHoliday;
    });
  ...
  async getHolidays() {
    let response = await fetch(
      "https://opendata.rijksoverheid.nl/v1/sources/rijksoverheid/infotypes/schoolholidays/schoolyear/2020-2021?output=json"
    );
