...
class MyApp extends Homey.App {
  ...
  async onInit() {
    ...
    isHolidayCondition.registerRunListener(async (args, state) => {
      let isHoliday;
      isHoliday = this.filterRegion(args.region);
      console.log(1, isHoliday)
      return isHoliday;
    });
  }
  ...
  filterRegion(regionToFilter) {
    ...
    this.getHolidays().then((regions) => {
      ...
      console.log('pd', this.processData(regions, regionToFilter))
      }
    );
    console.log(2, returnDates)

    // this.filterRegion(args.regio).then((dates) => {
    //   setTimeout(() => {
    //     dates.push("2021-04-05")
    //     isHoliday = this.isHoliday(dates);
    //     this.log(1, isHoliday)
    //     return
    //   }, 1)
    // }).catch(err => console.error(err));
    // this.log(2, isHoliday)
